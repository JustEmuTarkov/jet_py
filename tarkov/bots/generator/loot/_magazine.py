...
class BotMagazineGenerator(BaseLootGenerator):
    ...
    def generate_magazines_for_weapon(self, weapon: Item) -> None:
        ...
        ammo_types: List[TemplateId] = self.preset.inventory["mods"][magazine_template.id]["cartridges"]
        ...
        if reload_type == MagazineReloadType.ExternalMagazine:
            for _ in range(random.randint(self.config.magazines.min, self.config.magazines.max)):
                for _ in range(self.ATTEMPTS_TO_PLACE):
                    container = self.inventory_containers.random_container(include=["Pockets", "TacticalVest"])
